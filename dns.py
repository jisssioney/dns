...
- Resolver(zone, plan, timeout=5): 权威缓存与上游转发组合的解析器，
  ...
  c[0] 于下次解析提交时同步），每次成功按新修订号归档区域深拷贝；
...
_RCODE_NXDOMAIN = 3
_CACHE_CAPACITY = 256
# Resolver 区域修订历史容量：仅保留最近 32 个修订的规范化区域深拷贝，
# 超量淘汰最小修订号，修订号不复用。
...
class Resolver:
    ...
    构造时将规范化初始区域存为修订 0 的历史快照；reload_zone 成功或
    reload_zone_tx 返回 applied 时按新当前修订号保存区域深拷贝，其余
    结果与异常不保存。历史容量 32，超量淘汰最小修订号，号码不复用。

    rollback_zone_tx(target, expected)：带修订号检查的原子回滚事务，
    返回键序仅 version,result,target 的紧凑 ASCII JSON 报告（末尾换行）。
    target/expected 任一非 int（含 bool）抛 TypeError，负值抛
    ConfigError；两参数校验后比较 expected：不等于当前修订号时不得查询
    target，报告 conflict；相等且 target 为当前号报告 unchanged，target
    未保留报告 missing。命中时先以快照构造 PositiveCache，成功后原子
    换区、清空权威缓存，修订号加 1 并归档恢复区域，报告 applied；保留
    递归缓存、时钟、plan 及统计（stats() 提交当下逐字节不变）。
    非 applied 结果或异常不改变任何状态。
    """
    ...
    def __init__(self, zone: dict, plan: list, timeout: int = 5):
        ...
        self._stats_c0 = 0
        self._revision = 0  # 当前修订号；下次成功换区/回滚后加 1
        # 区域修订历史：修订号 -> 规范化 zone 深拷贝，容量 32，超量淘汰
        # 最小修订号（号码不复用）。构造时先存修订 0。
        self._history = {}
        self._archive_zone(0, self._snapshot_zone())

    def _snapshot_zone(self):
        """从当前权威缓存规范化内容构造 zone 模型的深拷贝。"""
        return {"origin": _labels_to_name(self._cache._origin),
                "records": [copy.deepcopy(_rr_to_model(rr))
                            for rr in self._cache._records]}

    def _archive_zone(self, revision, zone):
        """按 revision 保存 zone 深拷贝并按容量 32 淘汰最小修订号。"""
        self._history[revision] = copy.deepcopy(zone)
        if len(self._history) > _ZONE_HISTORY_CAPACITY:
            del self._history[min(self._history)]

    def _commit_zone(self, cache):
        """原子换区提交点：替换权威缓存、修订号加 1 并归档新区域。

        cache 已由调用方构造成功；换区清空权威缓存条目，递归缓存、时钟、
        plan 与统计均保留（统计不随提交更新）。
        """
        self._cache = cache
        self._revision += 1
        self._archive_zone(self._revision, self._snapshot_zone())
    ...
    def reload_zone(self, text: str) -> int:
        ...
        提交时与新缓存同步；并按新修订号归档区域深拷贝（容量 32，超量
        淘汰最小修订号）。任何失败（TypeError、ConfigError、RecordError、
        ZoneError）都回滚：不改变任何状态（含修订历史）。
        """
        zone = import_zone(text)
        cache = PositiveCache(zone)
        revision = self._revision
        self._commit_zone(cache)
        return revision
    ...
    def reload_zone_tx(self, text: str, expected: int) -> str:
        ...
        不变（不归档）；否则校验成功后原子换区（清空权威缓存，保留
        递归缓存、时钟、plan 与统计），修订号加 1 并按新修订号归档
        区域深拷贝，报告 applied（version 为新修订号，提交当下 stats()
        逐字节不变）。修订号与 reload_zone 共用，初始为 0。报告为紧凑
        ...
        current_text = export_zone(self._snapshot_zone())
    ...
    def rollback_zone_tx(self, target: int, expected: int) -> str:
        ...
        target、expected 任一非 int（含 bool）抛 TypeError，任一为负抛
        ConfigError；两参数校验全部完成后才比较 expected：expected 不
        等于当前修订号时不得查询 target，报告 conflict（version 为当前
        修订号），不改变任何状态。相等时 target 为当前修订号报告
        unchanged；target 快照未保留（已被容量淘汰或从未存在）报告
        missing。命中时先以 target 快照构造 PositiveCache，成功后才原子
        换区（清空权威缓存，保留递归缓存、时钟、plan 与统计，stats()
        提交当下逐字节不变），修订号加 1 并按新修订号归档恢复区域，
        报告 applied（version 为新修订号）。构造失败沿用异常且不改变
        任何状态。报告为紧凑 ASCII JSON、十进制数字、末尾单换行，
        result 为 "applied"、"unchanged"、"missing" 或 "conflict"，
        target 回显请求的 target。
        """
